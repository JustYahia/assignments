let tasksArray = [
  ...
  let nameRepetitions = new Map();
  
 
  for (let i = 0; i < tasksArray.length; i++) {
    let name = tasksArray[i][0];
  
    
    uniqueNames.add(name);
  
    
    if (nameRepetitions.has(name)) {
      let count = nameRepetitions.get(name);
      nameRepetitions.set(name, count + 1);
    } else {
      nameRepetitions.set(name, 1);
    }
  }
  
  console.log("Unique Names:", uniqueNames);
  console.log("Name Repetitions:", nameRepetitions);
